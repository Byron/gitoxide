...
mod keys {
    ...
    fn string() -> crate::Result {
        assert_eq!(
            git::config::tree::Http::USER_AGENT.try_into_string(bcow("agent"))?,
            "agent"
        );
        assert!(git::config::tree::Http::USER_AGENT.validate("agent".into()).is_ok());

        let invalid = b"\xF0\x80\x80".as_bstr();
        assert_eq!(
            git::config::tree::Http::USER_AGENT
                .try_into_string(Cow::Borrowed(invalid))
                .unwrap_err()
                .to_string(),
            "The utf-8 string at \"http.userAgent=���\" could not be decoded"
        );
        assert!(git::config::tree::Http::USER_AGENT.validate(invalid).is_err());
    ...
    fn any() {
        assert!(
            !git::config::Tree.sections().is_empty(),
...
mod checkout {
    use git_repository::config::tree::{Checkout, Key};
...
mod pack {
    use git_repository::config::tree::{Key, Pack};
