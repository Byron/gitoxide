...
mod index_version {
    ...
    impl IndexVersion {
        ...
        ) -> Result<git_pack::index::Version, config::key::GenericError> {
            let value = value.map_err(|err| config::key::GenericError::from(self).with_source(err))?;
            Ok(match value {
                1 => git_pack::index::Version::V1,
                2 => git_pack::index::Version::V2,
...
mod validate {
    ...
    impl keys::Validate for IndexVersion {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
            super::Pack::INDEX_VERSION.try_into_index_version(git_config::Integer::try_from(value).and_then(
                |int| {
                    int.to_decimal()
                        .ok_or_else(|| git_config::value::Error::new("integer out of range", value))
                },
            ))?;
            Ok(())
        }
