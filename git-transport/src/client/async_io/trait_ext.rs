...
pub trait TransportV2Ext {
    ...
    async fn invoke<'a>(
        &mut self,
        command: &str,
        capabilities: impl IntoIterator<Item = (&'a str, Option<&'a str>)> + Send,
        arguments: Option<impl IntoIterator<Item = bstr::BString> + Send>,
...
impl<T: Transport + Send> TransportV2Ext for T {
    async fn invoke<'a>(
        ...
        command: &str,
        capabilities: impl IntoIterator<Item = (&'a str, Option<&'a str>)> + Send,
        arguments: Option<impl IntoIterator<Item = BString> + Send>,
    ) -> Result<Box<dyn ExtendedBufRead + '_>, Error> {
        let mut writer = self
