...
pub fn find_git_repository_workdirs(
...
) -> anyhow::Result<Vec<(PathBuf, gix::repository::Kind)>> {
    progress.init(None, progress::count("filesystem items"));
    fn is_repository(path: &Path, is_dir: bool) -> Option<gix::repository::Kind> {
        if is_dir {
    ...
    struct Delegate<'a> {
        path: PathBuf,
        entries: &'a std::sync::Mutex<Vec<(PathBuf, gix::repository::Kind)>>,
        seen: &'a AtomicUsize,
        debug: bool,
    }
                ...
                Ok(dent) => {
                    ...
                    if let Some(kind) = is_repository(&self.path, dent.file_type().is_dir()) {
                        self.entries
                            .lock()
                            .unwrap()
                            .push((into_workdir(self.path.clone(), kind), kind));
                        WalkState::Skip
                    } else {
