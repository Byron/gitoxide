use crate::{keys, Credential, Key, Section, Tree};

impl Credential {
    /// The `credential.helper` key.
    pub const HELPER: keys::Program = keys::Program::new_program("helper", &Tree::CREDENTIAL);
    /// The `credential.username` key.
    pub const USERNAME: keys::Any = keys::Any::new("username", &Tree::CREDENTIAL);
    /// The `credential.useHttpPath` key.
    pub const USE_HTTP_PATH: keys::Boolean = keys::Boolean::new_boolean("useHttpPath", &Tree::CREDENTIAL);
