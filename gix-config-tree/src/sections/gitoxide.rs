use crate::{keys, Gitoxide, Key, Section, Tree};
...
impl Gitoxide {
    ...
    pub const USER_AGENT: keys::Any = keys::Any::new("userAgent", &Tree::GITOXIDE).with_note(
    ...
    pub const TRACE_PACKET: keys::Boolean = keys::Boolean::new_boolean("tracePacket", &Tree::GITOXIDE)
...
mod subsections {
    use crate::{sections::http, keys, Gitoxide, Key, Section, Tree};
    ...
    impl RefsNamespace {
        ...
        ) -> Result<gix_ref::Namespace, crate::refs_namespace::Error> {
            gix_ref::namespace::expand(name.as_ref())
                .map_err(|err| crate::key::Error::from_value(self, name.into_owned()).with_source(err))
        }
    }
