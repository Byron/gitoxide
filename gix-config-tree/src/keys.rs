...
use crate::{Key, Link, Note, Section, SubSectionRequirement};
...
impl<T: Validate> Any<T> {
    ...
    ) -> Result<gix_refspec::RefSpec, super::refspec::Error> {
        gix_refspec::parse(value.as_ref(), op)
            .map(|spec| spec.to_owned())
            .map_err(|err| super::refspec::Error::from_value(self, value.into_owned()).with_source(err))
    }

    /// Try to interpret `value` as UTF-8 encoded string.
    pub fn try_into_string(&'static self, value: Cow<'_, BStr>) -> Result<std::string::String, crate::string::Error> {
        use bstr::ByteVec;
        Vec::from(value.into_owned()).into_string().map_err(|err| {
            let utf8_err = err.utf8_error().clone();
            super::string::Error::from_value(self, err.into_vec().into()).with_source(utf8_err)
        })
    }
}
...
impl<T: Validate> Key for Any<T> {
    ...
    fn validate(&self, value: &BStr) -> Result<(), crate::key::validate::Error> {
...
mod duration {
    ...
    use crate::{keys::DurationInMilliseconds, Section};
    ...
    impl DurationInMilliseconds {
        ...
        ) -> Result<std::time::Duration, crate::duration::Error> {
            let value = value.map_err(|err| crate::duration::Error::from(self).with_source(err))?;
            Ok(match value {
...
mod lock_timeout {
    ...
    use crate::{keys::LockTimeout, Section};
    ...
    impl LockTimeout {
        ...
        pub fn try_into_lock_timeout(
            &'static self,
            value: Result<i64, gix_config_value::Error>,
        ) -> Result<gix_lock::acquire::Fail, crate::lock_timeout::Error> {
            let value = value.map_err(|err| crate::lock_timeout::Error::from(self).with_source(err))?;
            Ok(match value {
...
mod url {
    use std::borrow::Cow;
    use bstr::BStr;
    use crate::{keys::{validate, Url}, Section};
    ...
    impl Url {
        ...
        pub fn try_into_url(&'static self, value: Cow<'_, BStr>) -> Result<gix_url::Url, super::super::url::Error> {
            gix_url::parse(value.as_ref())
                .map_err(|err| super::super::url::Error::from_value(self, value.into_owned()).with_source(err))
        }
    }
}
...
mod workers {
    use crate::{keys::UnsignedInteger, Section};
    ...
    impl UnsignedInteger {
        ...
        pub fn try_into_usize(
            &'static self,
            value: Result<i64, gix_config_value::Error>,
        ) -> Result<usize, crate::unsigned_integer::Error> {
            value
                .map_err(|err| crate::unsigned_integer::Error::from(self).with_source(err))
                .and_then(|value| {
                    value
                        .try_into()
                        .map_err(|_| crate::unsigned_integer::Error::from(self))
                })
        }
        ...
        pub fn try_into_u64(
            &'static self,
            value: Result<i64, gix_config_value::Error>,
        ) -> Result<u64, crate::unsigned_integer::Error> {
            value
                .map_err(|err| crate::unsigned_integer::Error::from(self).with_source(err))
                .and_then(|value| {
                    value
        ...
        pub fn try_into_u32(
            &'static self,
            value: Result<i64, gix_config_value::Error>,
        ) -> Result<u32, crate::unsigned_integer::Error> {
            value
                .map_err(|err| crate::unsigned_integer::Error::from(self).with_source(err))
                .and_then(|value| {
                    value
...
mod time {
    ...
    use crate::{keys::{validate, Time}, Section};
...
mod boolean {
    use crate::{keys::{validate, Boolean}, Section};
    ...
    impl Boolean {
        ...
        ///
        /// `value` is expected to be provided by [`crate::File::boolean()`].
        pub fn enrich_error(
            &'static self,
            value: Result<bool, gix_config_value::Error>,
        ) -> Result<bool, crate::boolean::Error> {
            value.map_err(|err| crate::boolean::Error::from(self).with_source(err))
        }
    }
}
...
mod remote_name {
    ...
    use crate::{keys::RemoteName, Section};
    ...
    impl RemoteName {
        ...
        pub fn try_into_symbolic_name(
            &'static self,
            name: Cow<'_, BStr>,
        ) -> Result<BString, crate::remote::symbolic_name::Error> {
            crate::remote_name::validated(name.into_owned())
                .map_err(|err| crate::remote::symbolic_name::Error::from(self).with_source(err))
        }
    }
}
...
pub mod validate {
    ...
    impl Validate for UnsignedInteger {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn Error + Send + Sync + 'static>> {
            usize::try_from(
                gix_config_value::Integer::try_from(value)?
                    .to_decimal()
    ...
    impl Validate for Boolean {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn Error + Send + Sync + 'static>> {
            gix_config_value::Boolean::try_from(value)?;
            Ok(())
        }
    }
    ...
    impl Validate for RemoteName {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
            crate::remote_name::Name::try_from(Cow::Borrowed(value))
    ...
    impl Validate for DurationInMilliseconds {
        fn validate(&self, value: &BStr) -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>> {
            let value = gix_config_value::Integer::try_from(value)?
                .to_decimal()
                .ok_or_else(|| format!("integer {value} cannot be represented as integer"));
            crate::sections::gitoxide::Http::CONNECT_TIMEOUT.try_into_duration(Ok(value?))?;
            Ok(())
        }
    }
