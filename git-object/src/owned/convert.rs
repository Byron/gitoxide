...
impl From<borrowed::Tag<'_>> for owned::Tag {
    fn from(other: borrowed::Tag<'_>) -> owned::Tag {
        ...
        owned::Tag {
            target: git_hash::ObjectId::from_40_bytes_in_hex(&target).expect("40 bytes hex sha1"),
            name: name.to_owned(),
            target_kind,
...
impl From<borrowed::Commit<'_>> for owned::Commit {
    fn from(other: borrowed::Commit<'_>) -> owned::Commit {
        ...
        owned::Commit {
            tree: git_hash::ObjectId::from_40_bytes_in_hex(&tree).expect("40 bytes hex sha1"),
            parents: parents
                .iter()
                .map(|parent| git_hash::ObjectId::from_40_bytes_in_hex(parent).expect("40 bytes hex sha1"))
                .collect(),
            author: author.into(),
