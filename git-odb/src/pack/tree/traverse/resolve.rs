...
pub(crate) fn deltas<T, F, P, MBFN>(
...
where
    ...
    P: Progress,
    MBFN: for<'r> Fn(&'r mut T, &pack::data::Entry, &'r [u8]),
    T: Default,
{
    let mut decompressed_bytes_by_pack_offset = BTreeMap::new();
    let bytes_buf = RefCell::new(bytes_buf);
    let mut num_objects = 0;
    let decompress_from_resolver = |slice: EntrySlice| -> Result<(pack::data::Entry, Vec<u8>), Error> {
        ...
        Ok((entry, decompress_all_at_once(compressed, decompressed_len)?))
    };
    ...
    while let Some((level, mut base)) = nodes.pop() {
        let (base_entry, base_bytes) = if level == root_level {
        ...
        modify_base(&mut base.data, &base_entry, &base_bytes);
        num_objects += 1;
        for child in base.store_changes_then_into_child_iter() {
            let (mut child_entry, delta_bytes) = decompress_from_resolver(child.entry_slice())?;
            ...
            decompressed_bytes_by_pack_offset
                .insert(child.offset(), (child_entry, fully_resolved_delta_bytes.to_owned()));
