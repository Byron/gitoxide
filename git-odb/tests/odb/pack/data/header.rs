...
fn encode_decode_roundtrip() -> crate::Result {
    ...
    let (version, num_objects) = git_odb::pack::data::header::decode(expected_encoded_header.try_into()?)?;
    let actual_encoded_header = git_odb::pack::data::header::encode(version, num_objects);
