use crate::parse::{event::List, Error, Event, Section};
...
impl List<'static> {
    ...
    pub fn from_path<P: AsRef<std::path::Path>>(path: P) -> Result<List<'static>, from_path::Error> {
        let mut bytes = vec![];
...
impl<'a> List<'a> {
    ...
    pub fn from_str(input: &'a str) -> Result<List<'a>, Error<'a>> {
    ...
    /// data succeeding valid `git-config` data.
    #[allow(clippy::shadow_unrelated)]
    pub fn from_bytes(input: &'a [u8]) -> Result<List<'a>, Error<'a>> {
...
impl<'a> TryFrom<&'a str> for List<'a> {
    type Error = Error<'a>;
...
impl<'a> TryFrom<&'a [u8]> for List<'a> {
    type Error = Error<'a>;
