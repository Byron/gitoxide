...
use git_config::parse::{event, section, Event};
...
#[test]
fn parse_empty() {
    assert_eq!(event::List::from_str("").unwrap().into_vec(), vec![]);
}
...
fn error() {
    let input = "[a_b]\n c=d";
    assert_eq!(
        event::List::from_str(input).unwrap_err().to_string(),
