...
pub(in crate::store_impl::file) fn path_to_name<'a>(path: impl Into<Cow<'a, Path>>) -> Cow<'a, BStr> {
    let path = git_features::path::into_bytes(path.into()).expect("valid UTF-8 conversion - fixing this needs something like FullNameOS to represent ill-formed names read from disk");
