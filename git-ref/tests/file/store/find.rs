mod existing {
    ...
    fn possible_inputs() -> crate::Result {
        ...
        store.find_loose(PartialNameRef::try_from(name.remote)?.join(name.branch)?.to_ref())?;
        store.find_loose(PartialNameRef::try_from("origin")?.join("main")?.to_ref())?;
        store.find_loose(PartialNameRef::try_from("origin")?.join(String::from("main"))?.to_ref())?;
        store.find_loose(PartialNameRef::try_from("origin")?.join("main")?.to_ref())?;
